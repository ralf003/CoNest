...
目标结构确定为“OpenClaw 基座 + 双向宿主适配 + Cordis 增强内核 + 可选生态包”，由独立控制面管理配置与变更。OpenClaw 继续负责原生会话、渠道、任务入口、授权与交付；Cordis 为这些工作流提供可以声明依赖、组合、替换和释放的服务体系。整个插件生态的覆盖以此为基础展开，不把 CoNest 收缩成 DSH 工具桥，也不把 OpenClaw 降为一个无差别的宿主列表条目。
...
5. **生态兼容通过可选画像与适配包实现。** DSH 服务、原生执行器和客户端依赖分别显式装配；需要完整原生语义的插件保留原生 DSH 宿主。增加生态不得向 Core 引入该生态的必需依赖。
...
图 1：OpenClaw 与 Cordis 增强内核构成必需主路径；DSH 和其他生态位于可选扩展区。Manager 负责治理，普通执行不经过每个管理模块。图中的分层是逻辑职责，实际进程归属见 3.4。上下箭头表示宿主接口向组件开放、组件贡献向宿主绑定；具体调用包含请求与结果两个方向。
...
| **CoNest Manager / Studio / CLI** | 描述与制品、配置和变更计划、权限、观测及管理入口 | 依赖管理契约，管理 Core 与可选运行环境；不以 DSH 市场为唯一目录来源 |
...
| 运行位置 | 职责 | 生命周期所有者 |
|---|---|---|
| **OpenClaw Gateway** | 原生会话、渠道、Loop、原生插件，以及薄 Connector | OpenClaw 原生管理器；已有 Gateway 不被自动接管 |
| **Cordis 增强 Runtime** | Core 与受管理增强组件；按依赖图复用服务 | CoNest 管理其进程组和组件图，继承现有 worker 的监督能力 |
| **CoNest Manager** | 目标配置、制品、操作记录、状态协调 | 独立常驻控制服务，不加载第三方插件；Studio/CLI 是它的客户端 |
| **可选 DSH / 外部运行环境** | 需要原生 Agent/Session、UI 或外部服务的生态插件 | 对应 Connector 与原生管理器分工，同一实例有唯一启停所有者 |

**无 DSH 的完整部署只需要 OpenClaw、Cordis 增强 Runtime 和 Manager。** DSH 包下载、模型配置、进程、客户端和市场同步全部是可选步骤。需要相同原生服务和共享状态的受信任组件可同组；版本冲突、凭据范围或信任域不同则分组。不按每次任务启动运行环境，也不为每个组件强制增加进程。

Manager 保存目标与实际状态，执行面使用已获准绑定直接交互。同进程 Cordis 服务复用依赖注入；跨进程和跨宿主才使用明确协议。Manager 不参与每次本地依赖查找，也不取代 OpenClaw 的任务组织和授权权威。

DSH 固定 composition 最终移到可选 Support Pack 的运行环境：可独立服务纳入增强 Runtime，需要原生会话与客户端的部分进入 DSH Host。OpenClaw 中保留公开 harness 注册和宿主回调适配。此迁移不作为无 DSH 增强模式成立的前提。
...
自建增强组件可直接进入 Core 管理的服务图，完全不经过 DSH 接入路线。生态插件的三条路线可以组合：一个 DSH 插件在原生 DSH Host 中运行，其检索接口可供 OpenClaw 调用，原生 UI 仍由 DSH 客户端显示。不能跨宿主的内部 hooks 可以继续在原环境工作，不必因此重写整个插件。
...
**安装 OpenClaw 增强组件：** Studio/CLI 选择自建包 → Manager 解析 Cordis 服务依赖与 OpenClaw 贡献 → 增强 Runtime 装配组件图 → Connector 绑定公开扩展点 → OpenClaw 原生任务直接使用新增贡献。此路径不下载或启动 DSH。
...
| 独立控制面与状态所有权 | 统一升级影响、配置诊断和恢复记录 | 增加 Manager 与持久操作记录；复杂度必须由实际管理需求支撑 |
...
| C：完整组件管理 | 建立 Manager、包/贡献描述、持久操作记录、配置与诊断；沿用 Loader 和版本图 | 完成自建完整插件的安装、配置、后台订阅、UI/后端绑定、升级及卸载，无 DSH 模式持续通过 |
| D：可选生态接入 | 建立 DSH Support Pack、宿主画像与原生 Connector；纳管现有 composition、会话、模型和审批适配 | 真实运行服务、会话/工作流、前后端组合等 DSH 插件；移除 Support Pack 后基础增强和原生 OpenClaw 继续运行 |
| E：组合发布与平台验证 | Studio 展示依赖、兼容范围、贡献状态和恢复计划；按配置发布固定制品 | 基础模式、DSH 服务模式、完整 DSH 模式分别有依赖锁、安装和运行验收，新增生态不破坏 Core |
...
故障回归保留现有版本化调用和授权机制，新增基础/生态模式切换、依赖退出、前后端版本错配、后台撤权、事件回环、升级中断、Manager 重启、原生宿主故障及数据迁移失败。先用本地模型 fixture 驱动真实宿主，真实模型与各平台制品分别验收。
