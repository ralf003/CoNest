...
状态：面向 `develop` 的架构。第二节记录 0.6.4 实现，其余章节仍包含目标能力；不改变 `main` 或既有安装包的能力声明。
...
核对基线为 `develop` 的 0.6.4。固定依赖为 OpenClaw 2026.9.2 和 DSH 0.1.0-rc.5。平台执行证据见版本发布说明；打包成功不等于 Windows 11 实测通过。

| 领域 | 0.6.4 已实现 | 仍需补齐 |
|---|---|---|
| 独立增强 | 通用组件注册不再静态导入 DSH；办公服务、检查、上下文组件可在禁止 DSH 模块加载时运行 | Core 与 Support Pack 的独立发行依赖仍未拆包；完整安装包仍携带 DSH |
| 进程模型 | Gateway + CoNest Host；Management 与受管 Runtime 位于同一 Host，管理器在可重建 Context 外 | 多 Runtime / 远程部署按需求扩展 |
| DSH 执行器 | Agent/Session、固定 DSH 组合迁入同一 Host；Gateway 用私有双向 RPC 转发获准工具、事件、审批与取消 | 固定组合尚未成为可任意安装的全生态 Support Pack |
| 状态所有权 | 搜索、读取、记忆组件与 DSH 组合都在 Host；读取观察仍通过显式回执交给组合的 Session | 两类组合使用不同 Cordis Context，尚非一张统一可热替换的服务图 |
| 组件更新 | 已接纳组件调用固定依赖图，新调用使用新图；候选失败保留当前图；依赖停用阻断消费者 | 此保证不覆盖整个 Agent 任务，也不表示 DSH 执行器组合已具备版本化热替换 |
| 宿主贡献 | 工具发现/调用、公开 hooks、上下文提供者；办公组件演示复用同一知识服务 | 作业、UI、provider、消息等完整贡献模型继续建设 |
| Studio | 展示组件状态；完整双 Loop 与无 DSH 模式；确定性办公场景 | 服务图编辑与评估后发布界面尚未实现 |

0.6.4 的可演示价值是：通过服务契约复用业务能力、管理依赖就绪与生命周期、按单次组件调用维护更新前后的版本一致性。这不等于 OpenClaw 原生没有插件管理、恢复或自学习能力。完整生态兼容、跨系统事务以及自主评估发布仍不能作为已交付能力宣传。

默认双进程指常驻应用进程。DSH Agent 与 Session 是 Host 内对象，不是额外操作系统进程；Bash、stdio MCP 等仍可按需启动工具子进程。演示启动器还承担本地确定性模型 HTTP 服务。
