...
async function acl(file,protect){
 ...
 const script=protect?common+`$a=Get-Acl -LiteralPath $p; $a.SetAccessRuleProtection($true,$false); foreach($r in @($a.Access)){$a.RemoveAccessRuleSpecific($r)}; foreach($id in $allowed){$who=[System.Security.Principal.SecurityIdentifier]::new($id); $r=[System.Security.AccessControl.FileSystemAccessRule]::new($who,'FullControl','ContainerInherit,ObjectInherit','None','Allow'); $a.AddAccessRule($r)}; Set-Acl -LiteralPath $p -AclObject $a;`:common+`$a=Get-Acl -LiteralPath $p; foreach($r in $a.Access){if($r.AccessControlType -eq 'Allow'){$id=$r.IdentityReference.Translate([System.Security.Principal.SecurityIdentifier]).Value; if($allowed -notcontains $id){throw 'Credential ACL permits another identity; use CoNest setup to create a private credentials directory'}}}`;
 await execute('powershell.exe',['-NoLogo','-NoProfile','-NonInteractive','-EncodedCommand',Buffer.from(script,'utf16le').toString('base64')],{env:{...process.env,CONEST_ACL_PATH:file},windowsHide:true,timeout:20000});
}
