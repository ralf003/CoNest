...
The existing 0.5.1 and 0.6.0 release archives remain unchanged. Promoting any qualified profile into a shipped component still requires a relocatable dependency bundle, production configuration/trust decisions, and fresh clean-install/host acceptance for that artifact. The useful next candidates are bounded read and explicit-root Skill data access; the native Skill/Todo/Plan workflows remain blocked on explicit host mappings. Market implementation is separate.
