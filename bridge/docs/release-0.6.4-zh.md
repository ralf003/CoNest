...
本版把 DSH 执行器从 Gateway 迁入 CoNest Host，并提供一个不启动 DSH 的 OpenClaw 办公组件演示。版本范围以本文为准，运行结果以对应提交的 CI 和本地检查为准。
...
Windows：由 [Windows integration](https://github.com/zyw02/CoNest/actions/workflows/windows.yml) 从源码构建目标包，在原生 Windows Server 2025 runner 安装并运行。须以对应提交的实际运行结果为准，不能仅依据工作流文件宣称通过。Windows 11 桌面验收尚需独立机器。
