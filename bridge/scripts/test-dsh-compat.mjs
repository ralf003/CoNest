...
import { packagePaths, sourceRoot } from '../experiments/dsh-compat/profile.mjs';
import { executionEnvironment } from '../dist/environment.js';
import { reportDirectory } from './report-path.mjs';

const root = fileURLToPath(new URL('..', import.meta.url));
const output = process.env.CONEST_REPORT_PROFILE ? path.join(reportDirectory, 'dsh-compatibility') : path.join(root, 'reports', 'dsh-compatibility');
...
const tracked = ['experiments/dsh-compat/profile.mjs', 'experiments/dsh-compat/compat.test.mjs', 'scripts/test-dsh-compat.mjs',
...
try {
  const result = await promisify(execFile)(process.execPath, ['--test', '--test-reporter=tap', '--test-timeout=30000',
    'experiments/dsh-compat/compat.test.mjs'], { cwd: root, env: executionEnvironment(), timeout: 45000, maxBuffer: 4000000 });
