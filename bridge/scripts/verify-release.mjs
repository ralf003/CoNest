...
const fingerprint = async (file, report) => {
  ...
  const files = listing.filter(member => !member.endsWith('/') && (
    ...
    ['package/package.json', 'package/openclaw.plugin.json', 'package/runtime-lock.json', 'package/bridge.config.example.json', 'package/conest.config.example.json'].includes(member)
  )).sort();
