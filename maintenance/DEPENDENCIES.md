...
| DSH / Cordis | [Development SDK release](https://github.com/zyw02/CoNest/releases/tag/dev-sdk-20260915); archive and manifest SHA-256 values in [sdk.lock.json](sdk.lock.json) |
| Other JavaScript/native libraries | Exact direct versions in `bridge/package.json`, resolved graph in `bridge/pnpm-lock.yaml` |
| node-pty patch | [Pinned patch](../bridge/patches/node-pty@1.1.0.patch), applied and checked by pnpm |
...
2. Export from the preserved source/manifests or restored SDK: `python3 maintenance/export-sdk.py --source .vendor/dsh --output /tmp/conest-sdk-export`.
...
5. Validate platform artifacts separately using the [packaging guide](../bridge/docs/installation.md). SDK restoration alone does not establish native-platform compatibility.
