...
tar -xJf node-v24.15.0-linux-x64.tar.xz -C "$conest_install_root/node"
conest_node="$conest_install_root/node/node-v24.15.0-linux-x64/bin/node"
...
exec "$conest_launch_root/node/node-v24.15.0-linux-x64/bin/node" "$conest_launch_root/launch.mjs" "$@"
START
